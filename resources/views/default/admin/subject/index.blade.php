                                ...
                                <div class="box-body">
                                    <table id="example1" class="table table-bordered table-striped">
                                        ...
                                        @foreach($subjectList as $subject)
                                            <tr>
                                                <td>{{ $sl++ }}</td>
                                                <td>{{ $subject->studentClass->class_name or 'Null' }}</td>
                                                <td>{{ $subject->classTeacher->translate($locale)? $subject->classTeacher->first_name.' '.$subject->classTeacher->last_name:$subject->classTeacher->translate($defaultLocale)->first_name.' '.$subject->translate($defaultLocale)->last_name }}</td>
                                                <td>{{ $subject->subject_name or 'Null' }}</td>
                                                <td>{{ $subject->subject_author or 'Null' }}</td>
                                                <td>{{ $subject->subject_code or 'Null' }}</td>
                                                <td>{{ $subject->subject_credit or 'Null' }}</td>
                                                <td>{{ $subject->status or 'Null' }}</td>
                                                <td>

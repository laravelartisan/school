...
//                'punch_out' => $punchOutTime,
    ...
    public function usersDailyTimesheet($id, $month,$year,Holyday $holyday, User $user)
    {
        ...
        return view(
            'default.admin.timesheet.user-daily-time-report_copy',
             compact(
                'reportForUsers',
                'year',
                'month',
     ...
     * @param User $user
     */
    public function determineSalaryForAllEmployee($year,$month,Holyday $holyday,User $user)
    {
        ...
        foreach($reportForUsers as $reportForUser){
            ...
            $overtimeSalaryPerSec = $overtimeSalary/$numberOfDays/24/60;
            $usersOvertimeSalaryForMonth = $overtimeSalaryPerSec*$totalOvertimeHour;

            $salaryCut = $this->getSalaryCut($user, $basicSalary);
            $salaryCutPerSec = $salaryCut/$numberOfDays/24/60;
            $bonus = $this->getBonus($user,$basicSalary);dd($bonus);
        }
       ...
       /* $id = 120;
        ...
        $basicSalary = $this->getCurrentBasic($userToGetSalary);
        $allowances = $this->getTotalAllowance($userToGetSalary,$basicSalary);
        $overtime = $this->getTotalOvertime($userToGetSalary,$basicSalary);
        $salaryCut = $this->getSalaryCut($userToGetSalary, $basicSalary);
        $bonus = $this->getBonus($userToGetSalary,$basicSalary);dd($bonus);*/

//        $netSalary = $basicSalary+$allowances+$overtime+$bonus-$salaryCut;

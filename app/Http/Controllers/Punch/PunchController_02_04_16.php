...
//        return view('default.admin.punches.insert-punch',compact('viewType','punchFlag'));
    ...
    public function uploadDeviceData(User $user,Requests\Validator $request, Punch $punch)
    {
        ...
        $usersWithPunchTime = $this->commonUploadDevice($user,$request);
        ...
        foreach($usersWithPunchTime as $userKey => $user){
            ...
            foreach($user->time as $time){
                ...
                $punchIsSaved = $punch->create($punchTime);
            }
        }
        return $punchIsSaved ? back()->withSuccess('Successfully uploaded and data saved'):back()->withErrors('Not uploaded or saved');
    }
     ...
     * @return User
     */
    private function commonUploadDevice( $user,$request)
    {
        $users = $user->all();
